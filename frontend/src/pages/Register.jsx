...
export default function Register() {
  ...
  return (
    <div className="hero-bg min-h-screen grid place-items-center px-6 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-3xl p-8">
          ...
          <p className="text-slate-600 mb-6 text-sm">Start detecting fake news in seconds.</p>

          <Button onClick={googleLogin} variant="outline" className="w-full h-11 rounded-xl mb-4" data-testid="google-signup-btn">
          ...
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <Input id="name" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} data-testid="register-name-input" className="mt-1.5 h-11 rounded-xl" />
            </div>
            <div>
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} data-testid="register-email-input" className="mt-1.5 h-11 rounded-xl" />
            </div>
            <div>
              <Label htmlFor="password">Password (min 6)</Label>
              <Input id="password" type="password" required minLength={6} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} data-testid="register-password-input" className="mt-1.5 h-11 rounded-xl" />
            </div>
            <Button type="submit" disabled={loading} className="w-full h-11 rounded-xl bg-blue-600 hover:bg-blue-700" data-testid="register-submit-btn">
